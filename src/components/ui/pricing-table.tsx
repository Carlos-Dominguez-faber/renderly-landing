...
function PricingTableCell({
  ...
  return (
    ...
    >
      {children === true ? (
        <CheckIcon aria-hidden="true" className="size-4 text-emerald-400" />
      ) : children === false ? (
        <MinusIcon
          aria-hidden="true"
...
function PricingTablePlan({
  ...
  return (
    <div
      className={cn(
        'relative h-full overflow-hidden rounded-lg border p-3 font-normal backdrop-blur-sm',
        className
      )}
      {...props}
    >
      <div className="flex items-center gap-2">
        <div className="flex items-center justify-center rounded-full border border-white/[0.1] p-1.5">
          {Icon && <Icon className="h-3 w-3" />}
        </div>
        <h3 className="font-mono text-sm text-white/60">{name}</h3>
        <Badge
          variant="secondary"
          className="ml-auto rounded-full border border-white/[0.06] px-2 py-0.5 text-[10px] font-normal"
        >
          {badge}
        </Badge>
      </div>
